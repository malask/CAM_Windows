...
class INTER_TCL_FILE {
	public:
		...
		string get_depth_list () {
			...
			total_list << "{tree_V " << tree_V_depth << "}\n";
...
hls::stream<ap_uint<11> > (&result))
{
	...
	else
	{
		...
		// "tree_V"
		char* tvin_tree_V = new char[50];
		aesl_fh.touch(AUTOTB_TVIN_tree_V);
		...
		// [[transaction]]
		sprintf(tvin_tree_V, "[[transaction]] %d\n", AESL_transaction);
		aesl_fh.write(AUTOTB_TVIN_tree_V, tvin_tree_V);

		sc_bv<24>* tree_V_tvin_wrapc_buffer = new sc_bv<24>[1024];
			...
			// bitslice(23, 0)
			{
				...
				// celement: tree.V(23, 0)
				{
					...
					for (int i_0 = 0; i_0 <= 1023; i_0 += 1)
					{
						...
						// ori_name              : tree[i_0]
						// sub_1st_elem          : 0
						// ori_name_1st_elem     : tree[0]
						// regulate_c_name       : tree_V
						// input_type_conversion : (tree[i_0]).to_string(2).c_str()
						if (&(tree[0]) != NULL) // check the null address if the c port is array or others
						{
							sc_lv<24> tree_V_tmp_mem;
							tree_V_tmp_mem = (tree[i_0]).to_string(2).c_str();
							tree_V_tvin_wrapc_buffer[hls_map_index].range(23, 0) = tree_V_tmp_mem.range(23, 0);
                                 	       hls_map_index++;
						}
					}
				}
		...
		{
			sprintf(tvin_tree_V, "%s\n", (tree_V_tvin_wrapc_buffer[i]).to_string(SC_HEX).c_str());
			aesl_fh.write(AUTOTB_TVIN_tree_V, tvin_tree_V);
		}

		tcl_file.set_num(1024, &tcl_file.tree_V_depth);
		sprintf(tvin_tree_V, "[[/transaction]] \n");
		aesl_fh.write(AUTOTB_TVIN_tree_V, tvin_tree_V);

		// release memory allocation
		delete [] tree_V_tvin_wrapc_buffer;
...
// [call_c_dut] ---------->
		...
		top_function(tree, nodo, relationship, fatherSearch, result);
     ...
     aesl_tmp_0.clear();
		...
		// release memory allocation: "tree_V"
		delete [] tvin_tree_V;
